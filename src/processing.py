from __future__ import annotations

import traceback
import sys

# ---------------------------------------------------------------------------
# Lazy-import state  (all None until first processing call)
# ---------------------------------------------------------------------------
_np = None
_Image = None
_ImageFilter = None


def _ensure_processing_deps():
    """Import numpy + PIL on first call; return (np, Image, ImageFilter)."""
    global _np, _Image, _ImageFilter
    if _np is None:
        import numpy as _local_np
        from PIL import Image as _local_Image, ImageFilter as _local_ImageFilter
        _np = _local_np
        _Image = _local_Image
        _ImageFilter = _local_ImageFilter
    return _np, _Image, _ImageFilter


# ---------------------------------------------------------------------------
# sklearn probe
# ---------------------------------------------------------------------------
_sklearn_available: bool | None = None


def sklearn_available() -> bool:
    """Return True if scikit-learn can be imported (cached after first call)."""
    global _sklearn_available
    if _sklearn_available is None:
        try:
            import sklearn  # noqa: F401
            _sklearn_available = True
        except Exception:
            _sklearn_available = False
    return _sklearn_available


def active_dither_method() -> str:
    return "Bayer (ordered)" if sklearn_available() else "Floyd–Steinberg"


# ---------------------------------------------------------------------------
# Public pipeline functions
# ---------------------------------------------------------------------------

def resample_image(img, scale: float):
    np, Image, ImageFilter = _ensure_processing_deps()
    if scale == 1.0:
        return img
    w, h = img.size
    new_size = (max(1, int(w * scale)), max(1, int(h * scale)))
    return img.resize(new_size, resample=Image.LANCZOS)


def blur_image(img, radius: float):
    np, Image, ImageFilter = _ensure_processing_deps()
    if radius <= 0:
        return img
    return img.filter(ImageFilter.GaussianBlur(radius))


def quantize_image(img, colors: int = 16, dither: bool = True):
    np, Image, ImageFilter = _ensure_processing_deps()
    img_rgb = img.convert("RGB")
    n = max(1, colors)
    dither_flag = Image.Dither.FLOYDSTEINBERG if dither else Image.Dither.NONE
    palette_img = img_rgb.quantize(colors=n, method=Image.Quantize.MEDIANCUT,
                                   dither=Image.Dither.NONE)
    quantized = img_rgb.quantize(colors=n, palette=palette_img, dither=dither_flag)
    return quantized.convert("RGBA")


# ---------------------------------------------------------------------------
# NumPy helpers
# ---------------------------------------------------------------------------

def _pil_to_numpy_rgb_alpha(img):
    np, Image, ImageFilter = _ensure_processing_deps()
    rgba = img.convert("RGBA")
    arr = np.asarray(rgba)
    h, w = arr.shape[:2]
    rgb = arr[..., :3].astype(np.float32)
    alpha = arr[..., 3].astype(np.uint8)
    return rgb.reshape(-1, 3), alpha.reshape(-1), (w, h)


def _numpy_rgba_to_pil(rgb_flat, alpha_flat, size):
    np, Image, ImageFilter = _ensure_processing_deps()
    w, h = size
    rgb = np.clip(rgb_flat, 0, 255).astype(np.uint8).reshape((h, w, 3))
    alpha = alpha_flat.astype(np.uint8).reshape((h, w))
    out = np.empty((h, w, 4), dtype=np.uint8)
    out[..., :3] = rgb
    out[..., 3] = alpha
    return Image.fromarray(out, mode="RGBA")


_BAYER_CACHE: dict = {}   # int -> np.ndarray; plain dict avoids forward-ref on np


def _bayer_matrix(order: int):
    np, _, _ = _ensure_processing_deps()
    if order in _BAYER_CACHE:
        return _BAYER_CACHE[order]
    m = np.array([[0.0]], dtype=np.float32)
    for _ in range(order):
        m = np.block([[4 * m,     4 * m + 2],
                      [4 * m + 3, 4 * m + 1]])
    n = (2 ** order) ** 2
    result = (m / n - 0.5).astype(np.float32)
    _BAYER_CACHE[order] = result
    return result


_DITHER_CHUNK = 65536


def _ordered_dither(rgb_hw3, palette, bayer_order: int = 3):
    np, _, _ = _ensure_processing_deps()
    h, w = rgb_hw3.shape[:2]
    k = len(palette)

    bayer = _bayer_matrix(bayer_order)
    bh, bw = bayer.shape
    tiles_h = -(-h // bh)
    tiles_w = -(-w // bw)
    tiled = np.tile(bayer, (tiles_h, tiles_w))[:h, :w]

    spread = 255.0 / (2.0 * k)
    perturbed = rgb_hw3 + spread * tiled[:, :, np.newaxis]
    perturbed = np.clip(perturbed, 0.0, 255.0)

    flat = perturbed.reshape(-1, 3)
    n = flat.shape[0]

    pal_sq = np.sum(palette ** 2, axis=1)
    labels = np.empty(n, dtype=np.int32)
    for start in range(0, n, _DITHER_CHUNK):
        chunk = flat[start: start + _DITHER_CHUNK]
        chunk_sq = np.sum(chunk ** 2, axis=1, keepdims=True)
        dists = chunk_sq - 2.0 * (chunk @ palette.T) + pal_sq
        labels[start: start + _DITHER_CHUNK] = np.argmin(dists, axis=1)

    rgb_q = np.clip(palette[labels], 0, 255).astype(np.uint8)
    return rgb_q.reshape(h, w, 3)


_KMEANS_CACHE: dict = {}


def quantize_kmeans(img, colors: int = 16, sample_rate: float = 0.1,
                    batch_size: int = 1000, random_state: int = 0,
                    dither: bool = False, cache: bool = True):
    """Quantize using MiniBatchKMeans; falls back to Pillow when sklearn absent."""
    np, Image, _ = _ensure_processing_deps()
    if colors <= 0:
        raise ValueError("colors must be >= 1")

    print(f"[processing] quantize_kmeans start: colors={colors}, dither={dither}, sample_rate={sample_rate}")
    sys.stdout.flush()

    try:
        from sklearn.cluster import MiniBatchKMeans
    except Exception:
        print("[processing] sklearn not available, falling back to Pillow quantize")
        sys.stdout.flush()
        return quantize_image(img, colors=colors, dither=dither)

    rgb_flat, alpha_flat, (w, h) = _pil_to_numpy_rgb_alpha(img)
    n_pixels = rgb_flat.shape[0]
    print(f"[processing] image size: {w}x{h}, pixels={n_pixels}")
    sys.stdout.flush()

    if colors == 1:
        mean_color = np.mean(rgb_flat, axis=0, keepdims=True).astype(np.uint8)
        rgb_q = np.repeat(mean_color, n_pixels, axis=0)
        return _numpy_rgba_to_pil(rgb_q, alpha_flat, (w, h))

    if sample_rate < 1.0:
        rng = np.random.RandomState(random_state)
        sample_n = max(min(int(n_pixels * sample_rate), n_pixels), 1)
        sample_idx = rng.choice(n_pixels, size=sample_n, replace=False)
        sample = rgb_flat[sample_idx]
    else:
        sample = rgb_flat

    centers = None
    if cache:
        sample_mean = tuple(np.round(sample.mean(axis=0)).astype(int).tolist())
        sample_var  = tuple(np.round(sample.var(axis=0)).astype(int).tolist())
        key = (colors, min(batch_size, max(1, sample.shape[0])),
               random_state, sample.shape[0], sample_mean, sample_var)
        centers = _KMEANS_CACHE.get(key)

    if centers is None:
        print(f"[processing] fitting MiniBatchKMeans on sample.shape={sample.shape}")
        sys.stdout.flush()
        try:
            kmeans = MiniBatchKMeans(
                n_clusters=colors,
                batch_size=min(batch_size, max(1, sample.shape[0])),
                random_state=random_state,
            )
            kmeans.fit(sample)
            centers = kmeans.cluster_centers_.astype(np.uint8)
            if cache:
                _KMEANS_CACHE[key] = centers
            print("[processing] kmeans fit complete")
            sys.stdout.flush()
        except Exception as e:
            print("[processing] kmeans fit failed:", e)
            traceback.print_exc()
            sys.stdout.flush()
            return quantize_image(img, colors=colors, dither=dither)

    if dither:
        rgb_hw3 = rgb_flat.reshape(h, w, 3)
        rgb_dithered = _ordered_dither(rgb_hw3, centers.astype(np.float32))
        rgb_q = rgb_dithered.reshape(-1, 3)
    else:
        diffs = rgb_flat[:, None, :] - centers[None, :, :]
        labels = np.argmin(np.sum(diffs * diffs, axis=2), axis=1)
        rgb_q = centers[labels]

    print("[processing] quantize_kmeans completed reconstruction")
    sys.stdout.flush()
    return _numpy_rgba_to_pil(rgb_q, alpha_flat, (w, h))


def process_image(img, scale: float = 1.0, blur_radius: float = 0.0,
                  colors: int = 16, dither: bool = True,
                  use_pillow_dither: bool = False):
    """Resample → blur → quantize pipeline."""
    img = img.copy()
    img = resample_image(img, scale)
    img = blur_image(img, blur_radius)
    if use_pillow_dither or not sklearn_available():
        img = quantize_image(img, colors=colors, dither=dither)
    else:
        try:
            img = quantize_kmeans(img, colors=colors, sample_rate=0.1,
                                  batch_size=1000, random_state=0, dither=dither)
        except Exception:
            img = quantize_image(img, colors=colors, dither=dither)
    return img
